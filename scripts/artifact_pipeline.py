    ...
    try:
        with zipfile.ZipFile(file_path) as zf:
            ...
            if 'docProps/core.xml' in zf.namelist():
                ...
                metadata['creator'] = root.find('.//dc:creator', ns).text if root.find('.//dc:creator', ns) is not None else "Unknown"
                metadata['created'] = root.find('.//dcterms:created', ns).text if root.find('.//dcterms:created', ns) is not None else "Unknown"
                metadata['modified'] = root.find('.//dcterms:modified', ns).text if root.find('.//dcterms:modified', ns) is not None else "Unknown"
                metadata['title'] = root.find('.//dc:title', ns).text if root.find('.//dc:title', ns) is not None else "Untitled"
            ...
            if 'word/document.xml' in zf.namelist():
                ...
                metadata['raw_text_sample'] = " ".join(paragraphs[:50]) + ("..." if len(paragraphs) > 50 else "")
                metadata['parsing_status'] = "Fallback (Metadata Only)"
        return metadata
    except Exception as e:
        logger.error(f"Zip metadata extraction failed: {e}")
        return {"error": f"Metadata extraction failed: {str(e)}", "parsing_status": "Failed"}
...
def parse_docx(file_path: str) -> dict:
    ...
    if not HAS_DOCX:
        logger.warning("python-docx library not found. Falling back to metadata extraction.")
    ...
    try:
        ...
        for p in doc.paragraphs:
            ...
            if text.startswith("SYS_REQ_") or text.startswith("REQ_"):
                ...
                current_req = {"id": req_id, "title": req_title, "description": "", "aspice_ref": "SYS.2"}
            elif current_req and text:
                if text.startswith("ASPICE Reference:"):
                    current_req["aspice_ref"] = text.replace("ASPICE Reference:", "").strip()
                else:
...
def parse_xlsx(file_path: str) -> dict:
    ...
    if not HAS_OPENPYXL:
        logger.warning("openpyxl library not found. Falling back to metadata extraction.")
        ...
        for r_idx, row in enumerate(ws.iter_rows(values_only=True)):
            ...
            if row_dict.get("id") or row_dict.get("requirement id"):
                requirements.append({
                    ...
                    "description": row_dict.get("description") or row_dict.get("text") or "",
                    "aspice_ref": row_dict.get("aspice_ref") or row_dict.get("aspice") or "SYS.2"
                })
        return {
            "parsing_status": "Success",
            "type": "xlsx",
        ...
        with open(file_path, mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                requirements.append({
                    ...
                    "aspice_ref": row.get("aspice_ref") or row.get("ASPICE Reference") or "SYS.2"
                })
        return {
            "parsing_status": "Success",
            "type": "csv",
...
def run_validation(repo_path: str) -> dict:
    ...
    try:
        ...
        lint_res = subprocess.run(lint_cmd, cwd=repo_path, capture_output=True, text=True)
    ...
    try:
        ...
        test_res = subprocess.run(pytest_cmd, cwd=repo_path, capture_output=True, text=True)
    ...
    try:
        async for message in websocket:
            ...
            if cmd == "parse":
                ...
                await websocket.send(json.dumps({"type": "status", "status": "parsing"}))
                await websocket.send(json.dumps({"type": "log", "message": f"Parsing file: {file_path}"}))
                ...
                await websocket.send(json.dumps({"type": "status", "status": "validating"}))
                await websocket.send(json.dumps({"type": "log", "message": "Starting verification (ruff check + pytest)..."}))
...
def main():
    parser = argparse.ArgumentParser(description="ASPICE Technical Systems Lead Artifact Pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)
    
    # Subcommand: parse
    p_parse = subparsers.add_parser("parse", help="Parse requirements artifact")
    p_parse.add_argument("--file", required=True, help="Path to docx/xlsx/csv artifact file")
    p_parse.add_argument("--output", required=True, help="Output path for JSON result")
    
    # Subcommand: validate
    p_val = subparsers.add_parser("validate", help="Run linting and test validation checks")
    p_val.add_argument("--repo-path", required=True, help="Path to agent-shell workspace root")
    p_val.add_argument("--output", required=True, help="Output path for JSON validation result")
    
    # Subcommand: serve-websocket
    p_serve = subparsers.add_parser("serve-websocket", help="Start the WebSocket server gateway")
    p_serve.add_argument("--port", type=int, default=8765, help="Port to run WebSocket server on")
