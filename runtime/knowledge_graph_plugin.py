...
class KnowledgeGraphPlugin(ToolPlugin):
            ...
            "parse_requirements_document": {
                ...
                "inputSchema": {
                    ...
                    "properties": {
                        "document_text": {
                            "type": "string",
                            "description": "The requirements document text"
                        },
                        "document_name": {
                            "type": "string",
    ...
    def _parse_requirements(self, document_text: str, document_name: str) -> Dict[str, Any]:
        ...
        if self.use_local and self.local_server:
            ...
            return self._call_remote_tool("parse_requirements", {
    ...
    def _build_graph(self, documents: Dict[str, str], graph_id: str) -> Dict[str, Any]:
        ...
        if self.use_local and self.local_server:
            ...
            return self._call_remote_tool("create_knowledge_graph", {
                "documents": documents,
                "graph_id": graph_id
            })
    ...
    def _generate_tasks(self, graph_id: str, parent_task: str, parent_task_id: str) -> Dict[str, Any]:
        ...
        if self.use_local and self.local_server:
            ...
            return self._call_remote_tool("assign_expertise_clusters", {
                "graph_id": graph_id,
        ...
        try:
            with httpx.Client(timeout=30) as client:
                response = client.post(
                    f"{self.mcp_server_url}/tool/{tool_name}",
                    json=tool_input
                )
            ...
            return {
                "status": "error",
                "message": f"Remote call failed: {str(e)}"
            }
