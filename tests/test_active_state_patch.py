...
def test_active_state_patch_preserves_prior_and_writes_run_id(tmp_path: Path):
    ...
    write_json(registry_path, {"active_registry_version": "reg-v1", "active_registry_path": "configs/tool_registry.json"})
    write_json(skills_path, {"active_skills_version": "skills-v1", "active_skill_paths": ["configs/skills/a.json"]})
...
def test_no_direct_mutation_of_non_pointer_files(tmp_path: Path):
    ...
    patch_active_registry_state(
        run_id="run-4",
        candidate_registry_bundle={"candidate_registry_version": "reg-v9", "candidate_registry_path": "candidate.json"},
        active_registry_state={"active_registry_version": "reg-v8", "active_registry_path": "configs/tool_registry.json"},
        active_registry_path=tmp_path / "configs/state/active_registry.json",
    )
    patch_active_skills_state(
        run_id="run-4",
        candidate_skills_bundle={"candidate_skills_version": "skills-v9", "candidate_skill_paths": ["candidate_skills.json"]},
        active_skills_state={"active_skills_version": "skills-v8", "active_skill_paths": ["configs/skills/alpha.json"]},
